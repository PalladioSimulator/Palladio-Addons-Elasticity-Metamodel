/**
 * Copyright 2022 by palladiosimulator.org
 */
package org.palladiosimulator.elasticity.triggers.stimuli.tests;

import org.palladiosimulator.elasticity.triggers.stimuli.ManagedElementsStateStimulus;

/**
 * <!-- begin-user-doc --> A test case for the model object '<em><b>Managed Elements State
 * Stimulus</b></em>'. <!-- end-user-doc -->
 *
 * @generated
 */
public abstract class ManagedElementsStateStimulusTest extends TargetGroupStateStimulusTest {

    /**
     * Constructs a new Managed Elements State Stimulus test case with the given name. <!--
     * begin-user-doc --> <!-- end-user-doc -->
     *
     * @generated
     */
    public ManagedElementsStateStimulusTest(final String name) {
        super(name);
    }

    /**
     * Returns the fixture for this Managed Elements State Stimulus test case. <!-- begin-user-doc
     * --> <!-- end-user-doc -->
     *
     * @generated
     */
    @Override
    protected ManagedElementsStateStimulus getFixture() {
        return (ManagedElementsStateStimulus) this.fixture;
    }

} // ManagedElementsStateStimulusTest
